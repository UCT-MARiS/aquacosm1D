...
for tau_mean in tau_means:
    for Qswmax in Qswmaxs:
        ...
        amplitudes = [0.03]#[0.01, 0.02,0.03, 0.04]
        for amplitude in amplitudes:
            ...
            kappa_croco_r[:,:] = np.flip(kappa_croco_r,1)  #w2rho flips z for some reason. need to flip kappa back
           # ax[1].plot(time_croco,kappa_croco_surf,label=str(amplitude))
            ax[1].plot(z,kappa_croco_r[700,:])
        ...
        ax[0].set_xticklabels(range(10,16))
        ax[0].set_xlim(0,22)
       # ax[0].set_ylim(50,0)
