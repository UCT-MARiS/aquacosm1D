...
def do_the_plot(mld,amplitude,mean_tau,Qswmax,React):
    ...
    # add the thermocline
    for n in range(0,3):
        cnt=ax[n].plot(time_croco,z_therm_croco,'w')
    ...
    # ps=[1e-3,1e-7]
    ps= 1e-7
    aqcfile='aquacosm_p'+"{0:1.0e}".format(p)+'_r'+str(React.MaxPhotoRate*(60.*60.*24.))+'_b'+str(react_params.BasalMetabolism)+'_c'+str(react_params.Chl_light_abs)+'_a'+str(react_params.CrowdingMortality)+'_l'+str(react_params.LightDecay)+'_mean'+str(mean_tau)+"_amp"+str(amplitude)+"_mld"+str(mld)+"_flx"+str(Qswmax)+'.nc'
    time_aqc,z_aqc,z_rank,chl_aqc,_ = get_aqc_output(aqcfile)
    ...
    ts.set_ylim(0,max(chl_aqc_avg.max(),chl_eul_avg.max()))
    ts.set_xlim(0,21)
    ...
    plt.savefig('plot_eul_aqc_2_r'+str(react_params.MaxPhotoRate)+'_b'+str(react_params.BasalMetabolism)+'_c'+str(react_params.Chl_light_abs)+'_a'+str(react_params.CrowdingMortality)+'_l'+str(react_params.LightDecay)+'_mean'+str(mean_tau)+"_amp"+str(amplitude)+"_mld"+str(mld)+"_flx"+str(Qswmax)+'.jpg',dpi=500,bbox_inches = 'tight')
